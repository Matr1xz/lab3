...
def combine_audio_video(video_path, audio_path, og_path):
    ...
    # os.system("ffmpeg-4.3.1-2020-10-01-full_build\\bin\\ffmpeg -i output\\combined_video_only.mkv -i \"%s\" -codec copy output\\combined_video_audio.mkv" % audio_path) # Combining the Frames and Audio into a Video
     # Important: Use raw string r"" for Windows path, and correct quotes
    frame_pattern = os.path.join(video_path, "%d.png")  # Auto correct path

    # Step 1: Combine frames into video
    cmd1 = f'ffmpeg -framerate {int(fps)} -i "{frame_pattern}" -c:v libx264 output/combined_video_only.mkv'
    os.system(cmd1)

    # Step 2: Combine video + audio
    cmd2 = f'ffmpeg -i output/combined_video_only.mkv -i "{audio_path}" -c:v copy -c:a copy output/combined_video_audio.mkv'
...
def get_frames(video_object, base_filename):
    ...
    # Thay đổi đường dẫn cho Ubuntu
    directory = os.path.join("output", f"{base_filename}_frames")
